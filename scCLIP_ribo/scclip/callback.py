...
class LossLoggingCallback(Callback):
    """Write per-batch losses to a CSV so NaNs/inf can be inspected after runs."""

    def __init__(self, log_every_n_steps: int = 1, filename: str = "loss_history.csv"):
        super().__init__()
    ...
    def on_fit_start(self, trainer, pl_module):
        log_dir = getattr(trainer.logger, "log_dir", None) or trainer.default_root_dir
        self.log_path = Path(log_dir) / self.filename
        self.log_path.parent.mkdir(parents=True, exist_ok=True)
        if not self.log_path.exists():
            self.log_path.write_text("mode,step,epoch,loss,is_nan\n", encoding="utf-8")

    def _write_loss(self, mode, trainer, loss):
        if loss is None or self.log_path is None:
            return
        ...
        with self.log_path.open("a", encoding="utf-8") as fh:
            fh.write(
                f"{mode},{trainer.global_step},{trainer.current_epoch},{value:.6f},{is_nan}\n"
            )
    ...
    def on_train_batch_end(
        ...
        if batch_idx % self.log_every_n_steps != 0:
            return
        self._write_loss("train", trainer, outputs)

    def on_validation_batch_end(
        self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=0
    ):
        self._write_loss("val", trainer, outputs)
